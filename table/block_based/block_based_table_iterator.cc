...
void BlockBasedTableIterator::SeekImpl(const Slice* target) {
  ...
  if (target) {
    ...
    // outfile << table_->get_rep()->file.get()->file_number() << "," << v.handle.ToString() << std::endl;
    // outfile.close();
  }
}
...
void BlockBasedTableIterator::Next() {
  ...
  // std::fstream outfile;
  // outfile.open("/mnt/sdc/tht/cache_exp/zipfian_999/data_block2.txt",std::ios::out|std::ios::app);
  // outfile << table_->get_rep()->file.get()->file_number() << "," << index_iter_->value().handle.ToString() << std::endl;
  // outfile.close();
}
