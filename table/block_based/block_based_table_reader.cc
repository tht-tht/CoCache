...
Status ReadBlockFromFile(
  ...
  auto start_time = ioptions.clock->NowNanos();
  ...
  auto end_time = ioptions.clock->NowNanos();
  return s;
}
...
Status BlockBasedTable::MaybeReadBlockAndLoadToCache(
  ...
  if (block_cache != nullptr || block_cache_compressed != nullptr) {
    ...
    if (block_entry->GetValue() == nullptr && !no_io && ro.fill_cache) {
      ...
      if (!contents) {
        ...
        auto start_time = rep_->ioptions.clock->NowNanos();
        s = block_fetcher.ReadBlockContents();
        auto end_time = rep_->ioptions.clock->NowNanos();
  ...
  if(block_type == BlockType::kData) {
    if(is_cache_hit) {
      RecordTick(rep_->ioptions.statistics, DATA_CACHE_HIT);
    }
    else {
      RecordTick(rep_->ioptions.statistics, DATA_CACHE_MISS);
    }
  }
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  auto start_time = rep_->ioptions.clock->NowNanos();
  if(get_context->ReadFilter()) {
    RecordTick(rep_->ioptions.statistics, FILTER_CACHE_MISS);
  }
  else {
    RecordTick(rep_->ioptions.statistics, FILTER_CACHE_HIT);
  }
  Status s;
  const bool no_io = read_options.read_tier == kBlockCacheTier;
  ...
  auto after_filter = rep_->ioptions.clock->NowNanos();
  get_context->AddFilterTime(after_filter - start_time);
  if (!may_match) {
    RecordTick(rep_->ioptions.statistics, BLOOM_FILTER_USEFUL);
    PERF_COUNTER_BY_LEVEL_ADD(bloom_filter_useful, 1, rep_->level);
    get_context->filter_useful = true;
  } else {
    get_context->filter_useful = false;
    if (get_context->ReadIndex()) {
      RecordTick(rep_->ioptions.statistics, INDEX_CACHE_MISS);
    } else {
      RecordTick(rep_->ioptions.statistics, INDEX_CACHE_HIT);
    }
    ...
    for (iiter->Seek(key); iiter->Valid() && !done; iiter->Next()) {
      ...
      // tht update: when access a data block, calculate the group index
      // and passed by getcontext;
      uint64_t group_index = v.handle.offset() / 4096 / 1024;
      get_context->group_index = group_index;
      ...
      if (done) {
        ...
        // if(does_referenced_key_exist) {
        //   std::fstream outfile;
        //   outfile.open("/mnt/sdc/tht/cache_exp/zipfian_999/data_block2.txt",std::ios::out|std::ios::app);
        //   outfile << rep_->file.get()->file_number() << "," << v.handle.ToString() << std::endl;
        //   outfile.close();
        // }
    ...
    for (auto miter = sst_file_range.begin(); miter != sst_file_range.end();
      ...
      do {
        ...
        if (done) {
          ...
          // std::fstream outfile;
          // outfile.open("/mnt/sdc/tht/cache_exp/zipfian_999/data_block2.txt",std::ios::out|std::ios::app);
          // outfile << rep_->file.get()->file_number() << "," << iiter->value().handle.ToString() << std::endl;
          // outfile.close();
          // Avoid the extra Next which is expensive in two-level indexes
          break;
        }
