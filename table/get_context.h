...
class GetContext {
 public:
  ...
  uint64_t read_filter_block_time_ = 0;

  uint64_t read_index_block_time_ = 0;

  uint64_t read_data_block_time_ = 0;

  uint64_t search_data_time_ = 0;

  int read_filter_num_ = 0;

  int read_index_num_ = 0;

  int read_data_num_ = 0;

  bool filter_useful = false;
  ...
  void AddFilterTime(uint64_t time) { filter_time_ += time; }
  
  uint64_t GetFilterTime() { return filter_time_; }

  bool ReadFilter() { return read_filter; }

  bool ReadIndex() { return read_index; }

  void SetReadFilter(bool i) { read_filter = i; }

  void SetReadIndex(bool i) { read_index = i; }

  // tht update
  void SetFileNumberAndGroupIndex(uint64_t filenumber_, uint64_t group_index_) {
    if(merge_context_) {
      merge_context_->file_number=filenumber_;
      merge_context_->group_index = group_index_;
    }
  }
