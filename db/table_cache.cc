...
TableCache::TableCache(const ImmutableCFOptions& ioptions,
  ...
  LRUCacheOptions co_filter;
  co_filter.strict_capacity_limit = true;
  co_filter.capacity = 1000000;
  co_filter.num_shard_bits = 1;
  co_filter.metadata_charge_policy = kDontChargeCacheMetadata;
  filter_cache_ = NewLRUCache(co_filter);
  LRUCacheOptions co_index;
  co_index.strict_capacity_limit = true;
  co_index.capacity = 1000000;
  co_index.num_shard_bits = 1;
  co_index.metadata_charge_policy = kDontChargeCacheMetadata;
  index_cache_ = NewLRUCache(co_index);
}
...
void TableCache::EraseHandle(const FileDescriptor& fd, Cache::Handle* handle) {
  ...
  cache_->Erase(key);
  filter_cache_.get()->Erase(key);
  index_cache_.get()->Erase(key);
  //filter_cache_.get()->SetCapacity(0);
}
  ...
  if (*handle == nullptr) {
    ...
    *handle = cache_->Lookup(key);
    ...
    if (!s.ok()) {
      ...
      int* file_number = new int(number);
      int* file_number2 = new int(number);
      filter_cache_.get()->Insert(key, file_number, 1, &DeleteEntry<int>);
      index_cache_.get()->Insert(key, file_number2, 1, &DeleteEntry<int>);
    }
    return s;
  }
  return Status::OK();
...
Status TableCache::Get(const ReadOptions& options,
  ...
  Cache::Handle* handle = nullptr;
  uint64_t number = fd.GetNumber();
  get_context->SetReadFilter(false);
  get_context->SetReadIndex(false);
  auto key = GetSliceForFileNumber(&number);
  if(cache_->Lookup(key) == nullptr) {
    get_context->SetReadFilter(true);
    get_context->SetReadIndex(true);

  }
  else {
    if (filter_cache_.get()->Lookup(key) == nullptr) {
      get_context->SetReadFilter(true);
      int* file_number = new int(fd.GetNumber());
      filter_cache_.get()->Insert(key, file_number, 1, &DeleteEntry<int>);
    }
    if(index_cache_.get()->Lookup(key) == nullptr) {
      get_context->SetReadIndex(true);
      int* file_number = new int(fd.GetNumber());
      index_cache_.get()->Insert(key, file_number, 1, &DeleteEntry<int>);
    }
  }
  if (!done) {
    assert(s.ok());
