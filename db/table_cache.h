...
class new_lru {
  ...
  public:
   int get(int key) { auto it = map.find(key);
   if(it == map.end()) {
     return -1;
   }
   ...
   void put(int key, int value) { auto it = map.find(key);
   if(it==map.end()) {
    if(cache.size() == cap) {
   ...
   void setcap(size_t cap_) { cap = cap_; }
...
class TableCache {
 public:
  ...
  void AdjustTwoCache(uint64_t tablecache_size) {
    filter_cache_.get()->SetCapacity(tablecache_size);
    index_cache_.get()->SetCapacity(tablecache_size);
  }
 private:
  ...
  std::shared_ptr<rocksdb::Cache> filter_cache_;
  std::shared_ptr<rocksdb::Cache> index_cache_;
};
