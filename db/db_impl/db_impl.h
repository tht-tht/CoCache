...
class DBImpl : public DB {
 ...
 private:
  ...
  void CompareAndAdjust(rocksdb::ColumnFamilyData *cfd,rocksdb::SuperVersion *sv);

  void CalculateAndAdapt(rocksdb::ColumnFamilyData *cfd,rocksdb::SuperVersion *sv);
  ...
  // tht update: hotness of all data blocks and metadata blocks
  std::unordered_map<uint64_t, std::unordered_map<uint64_t,uint64_t>> sst_data_hotness;
  std::unordered_map<uint64_t, uint64_t> sst_meta_hotness;

  std::unordered_map<uint64_t, std::unordered_map<uint64_t,uint64_t>> prev_sst_data_hotness;
  std::unordered_map<uint64_t, uint64_t> prev_sst_meta_hotness;

  std::atomic<uint64_t> last_adapt_seconds_;
  std::mutex vec_mutex_;

  struct compaction_results {
