...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
  ...
  last_adapt_seconds_.store(immutable_db_options_.clock->NowMicros() / kMicrosInSecond);
}
...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
  ...
  if (!done) {
    ...
    // tht update
    if(merge_context.file_number !=0 || merge_context.group_index != 0) {
      //std::lock_guard<std::mutex> lock(vec_mutex_);
      sst_meta_hotness[merge_context.file_number]++;
      sst_data_hotness[merge_context.file_number][merge_context.group_index]++;
    }
    uint64_t now_seconds = immutable_db_options_.clock->NowMicros() / kMicrosInSecond;
    if(now_seconds >= last_adapt_seconds_.load() + 60*5) {
      std::lock_guard<std::mutex> lock(vec_mutex_);
      if(now_seconds >= last_adapt_seconds_.load() + 60*5) {
        CalculateAndAdapt(cfd,sv);
        last_adapt_seconds_.store(UINT64_MAX-60*20);
        // CompareAndAdjust(cfd,sv);
      }
    }
  }
...
void DBImpl::CompareAndAdjust(rocksdb::ColumnFamilyData *cfd,rocksdb::SuperVersion *sv) {
  uint64_t prev_sst_size = prev_sst_meta_hotness.size();
  uint64_t changed_size = 0;
  for(auto prev_sst_meta : prev_sst_meta_hotness) {
    if(sst_meta_hotness.find(prev_sst_meta.first) == sst_meta_hotness.end()) {
      changed_size++;
    }
    else {
      if(abs(int(sst_meta_hotness[prev_sst_meta.first] - prev_sst_meta.second)) >= 0.2 * prev_sst_meta.second) {
        changed_size++;
      }
      else {
        uint64_t datablock_changed_size = 0;
        uint64_t prev_datablock_size = prev_sst_data_hotness[prev_sst_meta.first].size();
        for(auto prev_datablock : prev_sst_data_hotness[prev_sst_meta.first]) {
          if(sst_data_hotness[prev_sst_meta.first].find(prev_datablock.first) ==  sst_data_hotness[prev_sst_meta.first].end()) {
            datablock_changed_size++;
          }
          else {
            auto prev_datablock_hotness = prev_datablock.second;
            auto cur_datablock_hotness = sst_data_hotness[prev_sst_meta.first][prev_datablock.first];
            if(abs(int(prev_datablock_hotness-cur_datablock_hotness)) >= 0.2 * prev_datablock_hotness) {
              datablock_changed_size++;
            }
          }
        }
        if(datablock_changed_size >= 0.2 * prev_datablock_size) {
          changed_size++;
        }
      }
    }
  }
  if(changed_size >= 0.2 * prev_sst_meta_hotness.size()) {
    CalculateAndAdapt(cfd,sv);
  }
  prev_sst_data_hotness.clear();
  prev_sst_meta_hotness.clear();
  prev_sst_data_hotness = sst_data_hotness;
  prev_sst_meta_hotness = sst_meta_hotness;
  sst_data_hotness.clear();
  sst_meta_hotness.clear();
  uint64_t now_seconds = immutable_db_options_.clock->NowMicros() / kMicrosInSecond;
  last_adapt_seconds_.store(now_seconds);
}

void DBImpl::CalculateAndAdapt(rocksdb::ColumnFamilyData *cfd,rocksdb::SuperVersion *sv) {
  uint64_t memory_size = 1024 * 1024 * 5; // memory size: 2GB
  uint64_t datablock_size = 4; // data block size: 4KB group size=1024
  uint64_t group_size = 1024;
  uint64_t datablock_io = 1;     // data block io time: 1ms
  uint64_t metablock_size = 650;// meta block size: 1MB
  uint64_t metablock_io = 2; // meta block io time: 10ms
  uint64_t max_sst_nums = 7000;
  std::vector<uint64_t> candidates;
  std::vector<uint64_t> datablocks;
  std::vector<uint64_t> metablocks;
  for(auto sst: sst_meta_hotness) {
    metablocks.push_back(sst.second);
  }
  for(auto sst: sst_data_hotness) {
    for(auto db: sst.second) {
      datablocks.push_back(db.second);
    }
  }
  std::sort(metablocks.begin(),metablocks.end(),std::greater<uint64_t>());
  std::sort(datablocks.begin(),datablocks.end(),std::greater<uint64_t>());
  uint64_t max_misscosts = 0;
  uint64_t best_datablock_capacity = 8 * 1024;
  for (uint64_t size = 0; size <= memory_size; size += 1024 * 100) {
    uint64_t datablock_capacity = size;
    uint64_t metablock_capacity = memory_size - datablock_capacity;
    //uint64_t datablock_num = datablock_capacity/(datablock_size * group_size);
    uint64_t datablock_num = datablock_capacity/(datablock_size * group_size);
    uint64_t metablock_num = metablock_capacity/metablock_size;
    uint64_t misscosts = 0;
    for(uint64_t i = 0; i < std::min(max_sst_nums,metablock_num);i++) {
      misscosts += metablocks[i] * metablock_io;
    }
    for(uint64_t i = 0; i < std::min(datablocks.size(),datablock_num); i++) {
      misscosts += datablocks[i] * datablock_io;
    }
    if(misscosts > max_misscosts) {
      max_misscosts = misscosts;
      best_datablock_capacity = datablock_capacity;
    }
  }
  // todo: adjust block cache and table cache according to the result
  if(!cfd->IsDropped()) {
    auto* table_factory = cfd->ioptions()->table_factory;
    if(table_factory) {
      auto* bbto = table_factory->GetOptions<BlockBasedTableOptions>();
      if(bbto && bbto->block_cache) {
        //bbto->block_cache.get()->SetCapacity(best_datablock_capacity*1024);
      }
    }
  }
  //sv->current->SetTableCapacity((memory_size-best_datablock_capacity)/metablock_size);
  //sv->current->SetTableCapacity(7300);
  //ReturnAndCleanupSuperVersion(cfd, sv);
  std::fstream outfile;
  outfile.open("/mnt/sdc/tht/cache_management_exp/test/debug.txt",std::ios::out|std::ios::app);
  outfile << "\n" << best_datablock_capacity * 1024 << " "
          << (memory_size - best_datablock_capacity) / metablock_size;
}
