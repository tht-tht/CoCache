  ...
  *made_progress = false;
  ...
  if (!c) {
    ...
    for (const auto& f : *c->inputs(0)) {
      ...
      // tht update: erase the hotness of delelefiles
      // if(sst_data_hotness.find(f->fd.GetNumber()) != sst_data_hotness.end()) {
      //   sst_data_hotness.erase(f->fd.GetNumber());
      // }
      // if(sst_meta_hotness.find(f->fd.GetNumber()) != sst_meta_hotness.end()) {
      //   sst_meta_hotness.erase(f->fd.GetNumber());
      // }
    }
    ...
    *made_progress = true;
    ...
    mutex_.Lock();
    // tht update
    // std::vector<uint64_t> input_files(compaction_job_stats.num_input_files);
    // std::vector<uint64_t> output_files(compaction_job_stats.num_output_files);
    // compaction_job.InstallResult(input_files,output_files);
    // uint64_t sum_sst_meta_hotness = 0;
    // for(size_t i = 0; i < input_files.size(); i++) {
    //   if(sst_meta_hotness.find(input_files[i]) != sst_meta_hotness.end()) {
    //     sum_sst_meta_hotness += sst_meta_hotness[input_files[i]];
    //     sst_meta_hotness.erase(input_files[i]);
    //     sst_data_hotness.erase(input_files[i]);
    //   }
    // }
    // if(!output_files.empty()) {
    //   uint64_t new_sst_meta_hotness = sum_sst_meta_hotness / output_files.size();
    //   for(auto outf : output_files) {
    //     sst_meta_hotness[outf] = new_sst_meta_hotness;
    //     size_t group_nums = 10;
    //     for(size_t i = 0; i < group_nums; i++) {
    //       sst_data_hotness[outf][i] = new_sst_meta_hotness / group_nums;
    //     }
    //   }
    // }
