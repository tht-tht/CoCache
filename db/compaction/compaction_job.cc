...
void CompactionJob::InstallResult(std::vector<uint64_t>& inputs, std::vector<uint64_t>& outputs) {
  for(const auto& sub_compact : compact_->sub_compact_states) {
    for(const auto& out : sub_compact.outputs) {
      outputs.push_back(out.meta.fd.GetNumber());
    }
  }
  for(size_t which = 0; which < compact_->compaction->num_input_levels(); which++) {
    for(const auto&f : *(compact_->compaction->inputs(which))) {
      inputs.push_back(f->fd.GetNumber());
    }
  }
}
